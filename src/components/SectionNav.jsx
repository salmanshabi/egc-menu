          ...
          <div className="grid w-full grid-cols-3 gap-1.5 px-2 py-2 sm:flex sm:w-max sm:gap-2 sm:px-3">
            {sections.map((s) => (
              <button
                ...
                className={`grid h-[48px] w-full min-w-0 cursor-pointer grid-rows-[20px_1fr] place-items-center rounded-xl px-1.5 py-1 text-center text-xs leading-snug font-extrabold transition-colors duration-200 sm:h-[54px] sm:w-[84px] sm:flex-shrink-0 sm:px-2 sm:text-xs ${
                  activeId === s.id
              ...
              >
                ...
                <span className="whitespace-normal line-clamp-2 self-start">{s.name[lang]}</span>
              </button>
            ))}
          </div>
        </div>
